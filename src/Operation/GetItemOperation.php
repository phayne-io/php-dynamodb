 ...
 */
class GetItemOperation extends Item\AbstractItemOperation
{
    #[Override]
    public function execute(): array
    {
        try {
            $item = [];
            $results = $this->dynamoDbClient->getItem($this->toArray());

            if (is_array($results['Item'])) {
                $item = (array)$this->marshaler->unmarshalItem($results['Item'], false);
            }
